"""
===========================================================================
CELL-BIN version BCR QC analysis (IGH / IGK / IGL)
===========================================================================
Analysis on cell bins: reads only need to fall within the cell bin polygon,
they do NOT need to fall on the nucleus mask (this is the only difference
from the nucleus-strict version).
...
Output directory structure:
  BASE_OUT/
    ...
    cellbin_IGK/
    cellbin_IGL/
    cellbin_sequence_heatmap/
    cellbin_coordinates_heatmap/
===========================================================================
"""
...
BASE_OUT = "/data/scratch/projects/punim1236/AAA_chen_here/AAA克隆_XCR/扫描高占比B/Cell_bin扫描高占比"
...
# Heatmap axis range (kept consistent with other versions)
SEQ_AXIS_MAX_IGH   = 8
SEQ_AXIS_MAX_IGK   = 10
...
# Subfolders
DIRS = {
    "IGH":      os.path.join(BASE_OUT, "cellbin_IGH"),
    "IGK":      os.path.join(BASE_OUT, "cellbin_IGK"),
    "IGL":      os.path.join(BASE_OUT, "cellbin_IGL"),
    "seq_hm":   os.path.join(BASE_OUT, "cellbin_sequence_heatmap"),
    "coord_hm": os.path.join(BASE_OUT, "cellbin_coordinates_heatmap"),
}
...
# =========================================================================
# Step 2: Per-cell statistics function constrained by cell-polygon
#         -> Returns both unique targetSeq count and unique coords count
# =========================================================================

def count_chain_cellbin(df, chain, C, B, eps=EPS):
    """
    Returns a DataFrame with one row per cell bin:
      cell_idx, cx, cy,
      n_unique_{chain}_targetSeq,
      n_unique_{chain}_coords
    Only counts reads falling within the cell polygon (no nucleus constraint).
    """
    ...
    for i in range(B.shape[0]):
        ...
        # bbox prefilter only, no nucleus constraint
        mask = (
            (px >= xmin) & (px <= xmax) &
            (py >= ymin) & (py <= ymax)
        )
...
def plot_chain_frequency(stats, chain, value_col, xlabel_kind, outdir):
    ...
    plt.ylabel(f"Number of unique {chain} {xlabel_kind} per cell bin",
               fontsize=13)
...
def plot_bin_dominant_composition(stats, rctd, chain, value_col,
    ...
    For each cell bin grouped by bin_cat (0/1/2/3/>=4), after matching with RCTD:
      ...
      - Merge all "B-prefixed" types into "B cells"
        (consistent with the bin20 / nucleus versions)
    """
    stats_q = stats.copy()
    ...
    # Merge B-prefixed types (already collapsed into "B cell" in prepare_rctd;
    # here we uniformly rename to "B cells")
...
def run_chain_analysis(chain, stats, rctd, outdir):
    print(f"\n--- {chain} single-chain analysis (cell bin) ---")
    # targetSeq
    ...
    out_csv = os.path.join(outdir, f"{chain}_cellbin_stats.csv")
...
def plot_igh_igk_heatmap(sweep_df, kind, outdir, overall_b_frac):
    ...
    ax.set_title(
        f"Dominant-B proportion by (IGH, IGK) {kind}  -  cell bin\n"
...
def run():
    ...
    # 4. Per-chain analysis (cell bin)
    stats_by_chain = {}
    for chain in CHAINS:
        print(f"=== {chain} (cell bin statistics) ===")
        stats = count_chain_cellbin(df, chain, C, B, EPS)
    ...
    print(f"\n=== IGH x IGK heatmap: unique targetSeq (cell bin) ===")
    ...
    print(f"\n=== IGH x IGK heatmap: unique coords (cell bin) ===")
